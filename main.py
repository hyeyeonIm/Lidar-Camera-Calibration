        ...
        if success:
            ...
            if len(results[0].boxes) > 0:
                for box in results[0].boxes:
                    ...
                    if lidar_points:
                        ...
                        cv2.putText(frame, text, (int(xyxy[0]), int(xyxy[1]) + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            ...
            if lidar_points:
                for (x, y) in lidar_points:
                    ...
                    cv2.circle(frame, (int(img_points[0]), int(img_points[1])), 3, (0, 0, 255), 1)

            # 이미지 출력
            cv2.imshow("YOLOv8 and LiDAR Projection", frame)
